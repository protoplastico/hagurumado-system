...
export async function generateMetadata({ params }: { params: { locale: string; slug: string } }) {
  ...
  if (!page) return { title: t(locale).guide.heading }

  const title = locale === 'ja' ? page.title.ja : (page.title.en ?? page.title.ja)
  const description = locale === 'ja' ? page.summary?.ja : page.summary?.en
  const ogImage = page.coverImage ? urlFor(page.coverImage).width(1200).height(630).fit('crop').auto('format').url() : undefined

  return {
    title,
    description,
    openGraph: { title, description, type: 'article', images: ogImage ? [ogImage] : undefined },
  }
}
